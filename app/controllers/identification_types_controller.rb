class IdentificationTypesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to identification_types_path, status: :see_other, notice: "Identification type was successfully destroyed." }
  ...
  private
    ...
    def set_identification_type
      @identification_type = IdentificationType.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def identification_type_params
      params.expect(identification_type: [ :name ])
    end
end
